...
codefile_file_ii_0::~codefile_file_ii_0()
{
}
...
codefile_file_ii_0::codefile_file_ii_0(const rcstring &a_filename) :
    codefile_file(a_filename)
{
    ...
    //
    // There was a bug in the way the II.0 compiler wrote the copyright
...
bool
codefile_file_ii_0::candidate(const void *vblock0ptr)
{
...
codefile_file_ii_0::pointer
codefile_file_ii_0::create(const rcstring &a_filename)
{
    return pointer(new codefile_file_ii_0(a_filename));
}
...
codefile_file_ii_0::pointer
codefile_file_ii_0::create(const rcstring &a_filename, byte_sex_t a_byte_sex)
{
    return pointer(new codefile_file_ii_0(a_filename, a_byte_sex));
}


codefile::pointer
codefile_file_ii_0::new_file_factory(const rcstring &outfilename)
    const
{
...
void
codefile_file_ii_0::block0::clear(void)
{
    DEBUG(2, "sizeof(*this) => %zd", sizeof(*this));
...
bool
codefile_file_ii_0::looks_like_valid_segment_table(const block0 &buffer,
    byte_sex_t byte_sex)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);

    //
    // The filler must be zero, because the II.0 compiler sets it to
    // zero, and so do we.
...
segment::pointer
codefile_file_ii_0::new_segment(const rcstring &name, int segnum,
...
void
codefile_file_ii_0::write_segment_dictionary(void)
{
    DEBUG(1, "%s {", __PRETTY_FUNCTION__);
    block0 block;
    ...
    //
    // We do it the same way as the II.0 compiler, even though it had a
    // bug, and failed to emit the length byte first.  Unlike the II.0
    ...
    if (temp_len > sizeof(block.notice))
        temp_len = sizeof(block.notice);
    memcpy(block.notice, temp.c_str(), temp_len);
...
segment::pointer
codefile_file_ii_0::get_segment(unsigned number)
    const
{
    unsigned pos = 0;
    for (;;)
...
void
codefile_file_ii_0::create_dataseg(const rcstring &name, int segnum, int nwords)
{
    assert(nwords > 0);
...
unsigned
codefile_file_ii_0::maximum_number_of_segments(void)
    const
{
    return 16;
}
...
codefile_file_ii_0::native_disassembler_factory(mtype_t mt, dislabel &where,
